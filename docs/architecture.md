...
### MCP Interface
The server implements the Model Context Protocol (MCP) version `2025-06-18`. It uses a `streamable-http` transport (as indicated in the `/mcp` GET endpoint) and handles `initialize`, `tools/list`, and `tools/call` methods via the `/mcp` POST endpoint.
